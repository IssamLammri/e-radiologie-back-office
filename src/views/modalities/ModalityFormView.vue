...
const modalityId = isEditing ? Number(route.params.id) : null;
...
onMounted(async () => {
  if (isEditing && modalityId) {
    try {
      const modality = await modalitiesApi.getAdminById(modalityId);
      form.value = {
        name: modality.name,
        code: modality.code,
      ...
      globalError.value = "Impossible de charger la modalité.";
    } finally {
      loading.value = false;
    }
  }
...
const handleSubmit = async () => {
  ...
  try {
    if (isEditing && modalityId) {
      await modalitiesApi.update(modalityId, form.value);
    } else {
      await modalitiesApi.create(form.value);
    }
...
<template>
  <div class="space-y-6 max-w-3xl mx-auto">
    ...
    <div class="flex items-center gap-4">
      <RouterLink to="/modalities" class="p-2 text-slate-400 hover:text-slate-600 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors">
        <ArrowLeft class="h-5 w-5" />
      </RouterLink>
      <h1 class="text-2xl font-bold tracking-tight text-slate-900">
        {{ isEditing ? 'Modifier la modalité' : 'Créer une modalité' }}
      </h1>
    </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input id="name" type="text" v-model="form.name" required placeholder="Ex: Imagerie interventionnelle"
          ...
          <div>
            ...
            <input id="code" type="text" v-model="form.code" required placeholder="Ex: INTERVENTIONAL"
                   class="appearance-none block w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all sm:text-sm uppercase font-mono" />
            <p v-if="error.code" class="mt-1 text-sm text-red-600">{{ error.code[0] }}</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="flex items-center h-full pt-6">
            <label class="relative flex items-center cursor-pointer">
              ...
              <span class="ml-3 text-sm font-medium text-slate-700">Modalité active</span>
            </label>
